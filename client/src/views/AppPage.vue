...
export default defineComponent({
  ...
  methods: {
    async getAllUsers() {
      ...
      const roles = uniqBy(users.map((user: any) => user.role), 'id');
      ...
      const simulation = d3
        ...
        .force("link", d3.forceLink(links).id((data: any) => data.id))
    ...
    node
        ...
        .select("circle + text")
        .attr("x", function() {
          return (this as any).parentElement!.getAttribute("cx");
        })
        .attr("y", function() {
          return (this as any).parentElement!.getAttribute("cy");
        });
  });
